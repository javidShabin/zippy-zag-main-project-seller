...
const RestaurantDetails = () => {
  ...
  const updateTheStatus = async ({ isOpen }) => {
    console.log(isOpen);
    try {
      const response = await axiosInstance.put(
        `/restaurant/update-restaurant/${restaurantId}`,
        { isOpen } // Pass isOpen as an object with a boolean value
      );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      ...
      <div className="mt-6 flex space-x-4">
        <button
          onClick={() => {
            updateTheStatus({ isOpen: true }); // Set isOpen to true for "open"
          }}
        ...
        <button
          onClick={() => {
            updateTheStatus({ isOpen: false }); // Set isOpen to false for "close"
          }}
